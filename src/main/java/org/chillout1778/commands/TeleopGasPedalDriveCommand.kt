...
class TeleopGasPedalDriveCommand(val driver: CommandXboxController): Command() {
    ...
    fun getDriveX() : Double {
        return Utils.deadZone(
            squareWithSign(driver.leftX * Swerve.Constants.MAX_VELOCITY * getDriveSpeed()),
            0.1
        )
    }
    ...
    fun getDriveY() : Double {
        return Utils.deadZone(
            squareWithSign(driver.leftY * Swerve.Constants.MAX_VELOCITY * getDriveSpeed()),
            0.1
        )
    }
    ...
    fun getDriveR() : Double {
        return Utils.deadZone(
            squareWithSign(driver.rightX * Swerve.Constants.MAX_ANGULAR_VELOCITY * getDriveSpeed()),
            0.1
        )
    }
